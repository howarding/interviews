...
// occurrence of */ should be ignored. (Here, occurrences happen in reading order: line by line from left to right.) To
// be clear, the string /*/ does not yet end the block comment, as the ending would be overlapping the beginning.
...
//There will be no control characters, single quote, or double quote characters. For example, source = "string s = "/*
// Not a comment. */";" will not be a test case. (Also, nothing else such as defines or macros will interfere with the
// comments.)
...
//Example 1:
//    Input:
//    source = ["/*Test program */", "int main()", "{ ", "  // variable declaration ", "int a, b, c;", "/* This is a
...
public:
    ...
    vector<string> removeComments(vector<string>& source) {
        ...
        for (string& line: source) {
            ...
            if (!inBlock) newLine = "";
            for (; i < line.size(); i++) {
                if (!inBlock && i < line.size() - 1 && line[i] == '/' && line[i + 1] == '*') {
                    inBlock = true;
                    i++;
                } else if (inBlock && i < line.size() - 1 && line[i] == '*' && line[i + 1] == '/') {
                    inBlock = false;
                    i++;
                } else if (!inBlock && i < line.size() - 1 && line[i] == '/' && line[i + 1] == '/') {
                    break;
                } else if (!inBlock) newLine.push_back(line[i]);
            }
            if (!inBlock && newLine.size() > 0) result.push_back(newLine);
        }
        return result;
    }
};
